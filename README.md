...
- The shared action-view contract for button-driven use cases now lives in [action-view.ts](src/shared-kernel/application/action-view.ts), with shared Discord rendering in [render-action-result.ts](src/app/discord/render-action-result.ts) and [render-action-button-rows.ts](src/app/discord/render-action-button-rows.ts).
