...
- `ROLLY_DATA_DIR`: Absolute or repo-relative path to your private `rolly-data` checkout. If omitted, the app tries `./rolly-data` and then `./example-data/rolly-data`.
...
- `src/commands/minigames/` contains the active dice-game commands.
- `src/commands/utility/self-update.ts` contains the only remaining utility command.
- `src/lib/minigames/` contains the dice systems, including random events, prestige, PvP, achievements, analytics, and temporary effects.

The `minigames` folder name is historical. In this repo it currently means dice-game domain code.
