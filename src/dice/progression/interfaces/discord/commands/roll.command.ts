...
import { handleBeginnerRollAfterRoll } from "../../../../onboarding/interfaces/discord/beginner-roll-graduation";
...
export const execute = async (interaction: ChatInputCommandInteraction): Promise<void> => {
  ...
  const rendered = renderDiceRollResult(result);
  ...
  await handleBeginnerRollAfterRoll({
    ...
    userId: interaction.user.id,
    didReachBeginnerMilestoneOnThisRoll: hasReachedBeginnerMilestone(
      result.newlyEarnedAchievementIds,
    ),
  });
