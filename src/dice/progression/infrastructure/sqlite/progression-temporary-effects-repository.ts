...
export const createSqliteProgressionTemporaryEffectsRepository = (
  db: SqliteDatabase,
): Pick<
...
> => {
  ...
  const getActiveDiceTemporaryEffects = ({
    ...
    nowMs = Date.now(),
    commandName,
  }: GetActiveDiceTemporaryEffectsInput): DiceTemporaryEffect[] => {
    purgeExpiredDiceTemporaryEffects(nowMs);
  ...
  const insertDiceTemporaryEffect = (input: {
    id: string;
    userId: string;
    effectCode: string;
    kind: DiceTemporaryEffectKind;
  ...
  const applyDiceTemporaryEffect = ({
    ...
    stackGroup,
    stackMode = "stack",
  }: ApplyDiceTemporaryEffectInput): DiceTemporaryEffect => {
    ...
    return db.transaction(() => {
      purgeExpiredDiceTemporaryEffects(Date.parse(nowIso));

      if (stackMode === "stack") {
        return insertDiceTemporaryEffect({
          id: randomUUID(),
          userId,
          effectCode,
          kind,
      ...
      const existingRow = getActiveEffectByStackGroup(userId, normalizedStackGroup, nowIso);
      if (!existingRow) {
        return insertDiceTemporaryEffect({
          id: randomUUID(),
          userId,
  ...
  const consumeDiceTemporaryEffectsForRoll = ({
    ...
    nowMs = Date.now(),
    effectCodes,
  }: ConsumeDiceTemporaryEffectsForRollInput): number => {
  ...
  const clearAllDiceTemporaryEffects = (userId: string): number => {
  ...
  const clearNegativeDiceTemporaryEffects = (
    ...
    const effects = getActiveDiceTemporaryEffects({
      userId,
      nowMs,
