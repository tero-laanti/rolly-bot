...
const getDiceBans = (db: SqliteDatabase, userId: string): Map<number, Set<number>> => {
  const rows = db
...
const clearDiceBan = (db: SqliteDatabase, userId: string, dieIndex: number): void => {
  db.prepare("DELETE FROM dice_bans WHERE user_id = ? AND die_index = ?").run(userId, dieIndex);
};

const clearUserDiceBans = (db: SqliteDatabase, userId: string): void => {
...
const getUserDiceAchievements = (db: SqliteDatabase, userId: string): DiceAchievementId[] => {
  const rows = db
    .prepare(
...
const clearUserDiceAchievements = (db: SqliteDatabase, userId: string): void => {
  db.prepare("DELETE FROM user_achievements WHERE user_id = ?").run(userId);
};

const getLastDiceRollAt = (db: SqliteDatabase): number | null => {
...
const setLastDiceRollAt = (db: SqliteDatabase, nowMs: number): void => {
