...
test("raids live runtime rejects run actions when the raid run is missing", async () => {
  const modulePaths = [
    "../../../shared/db",
    "./catalog-reader",
    "./discord/discord-raid-instance-provisioner",
  ...
  try {
    ...
    const interaction = {
      channelId: "panel-channel",
      customId: "raids:join-run:missing-run:1",
      guild: {
        members: {
          fetch: async () => ({
            roles: {
    ...
    assert.equal(handleRaidActionCalled, false);
    assert.deepEqual(repliedPayload, {
      content: "This raid run is no longer available.",
      ephemeral: true,
    });
  } finally {
    (sharedDb as { getDatabase: typeof sharedDb.getDatabase }).getDatabase = originalGetDatabase;
